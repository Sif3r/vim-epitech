...
function! s:Header_script_file(project_name, file_descr, scom, ccom, ecom)
...
function! s:Epi_Header()
    let s:project_name = s:Project_name()
    ...
    if s:scom[0] == '#' && s:scom[1] == '!'
        call s:Header_script_file(s:project_name, s:file_descr, s:scom, s:ccom, s:ecom)
    else
